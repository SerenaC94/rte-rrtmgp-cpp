...
namespace rrtmgp_kernel_launcher_cuda
{
	...
	void compute_tau_absorption(
			...
			Float* tau)
			{
				...
				//Comment if you do want to use my version
#define MINE
				//Comment if you do not want the optimized version
#define OPTIMIZED
				//Comment if you do not want results printed
//#define PRINTRES
				...
				std::cout<<"Ngas: "<<ngas<<std::endl;
				std::cout<<"Nflav: "<<nflav<<std::endl;
				std::cout<<"Ntemp: "<<ntemp<<std::endl;
				std::cout<<"Neta: "<<neta<<std::endl;
				std::cout<<"Nminor: "<<nminorlower<<std::endl;
				std::cout<<"Nminork: "<<nminorklower<<std::endl;
				std::cout<<"Idx_h2o: "<<idx_h2o<<std::endl;
				std::cout<<"Idx_tropo: "<<idx_tropo<<std::endl;
				std::cout<<"Gpoint_flavor: "<<gpoint_flavor[0]<<std::endl;
				std::cout<<"Kminor: "<<kminor_lower[0]<<std::endl;
				std::cout<<"Tau array: "<<ncol*nlay*ngpt<<std::endl;
				Float sum=0.;
				for(size_t i=0;i<ncol*nlay*ngpt;++i)
					sum+=tau[i];
				std::cout<<"Sum: "<<std::setprecision(9)<<sum<<std::endl;
#endif
				...
				std::cout<<"Elapsed time for CUDA kernel: "<<millis<<" [ms]"<<std::endl;
#else
				const auto dim=ncol*nlay*ngpt;
				Float* my_tau=(Float*)malloc(sizeof(Float)*dim);
				cudaMemcpy(my_tau,tau,sizeof(Float)*dim,cudaMemcpyDeviceToHost);

				const auto t0=std::chrono::high_resolution_clock::now();
#ifdef OPTIMIZED
				gas_optical_depths_minor_reference_optimized_kernel<1,42,8,8,1,16>(
						ncol, nlay, ngpt,
						ngas, nflav, ntemp, neta,
						nminorlower,
						nminorklower,
						...
						play, tlay, col_gas,
						fminor, jeta, jtemp,
						tropo, my_tau, nullptr);
#else 
				gas_optical_depths_minor_reference_kernel<1,42,8,8,1,16>(
						ncol, nlay, ngpt,
						ngas, nflav, ntemp, neta,
						nminorlower,
						nminorklower,
				...
				cudaMemcpy(tau,my_tau,sizeof(Float)*dim,cudaMemcpyHostToDevice);

				std::cout<<"Elapsed time for sequential kernel: "<<dt<<" [ms]"<<std::endl;
				...
				std::cout<<"Ntemp: "<<ntemp<<std::endl;
				std::cout<<"Neta: "<<neta<<std::endl;
				std::cout<<"Nminor: "<<nminorupper<<std::endl;
				std::cout<<"Nminork: "<<nminorkupper<<std::endl;
				std::cout<<"Idx_h2o: "<<idx_h2o<<std::endl;
				std::cout<<"Idx_tropo: "<<idx_tropo<<std::endl;
				std::cout<<"Gpoint_flavor: "<<gpoint_flavor[0]<<std::endl;
				std::cout<<"Kminor: "<<kminor_lower[0]<<std::endl;
...
#ifdef OPTIMIZED
				gas_optical_depths_minor_reference_optimized_kernel<1,42,4,8,1,32>(
						ncol, nlay, ngpt,
						ngas, nflav, ntemp, neta,
						nminorupper,
						nminorkupper,
						...
						tropo, my_tau, nullptr);
#else
				gas_optical_depths_minor_reference_kernel<1,42,4,8,1,32>(
						ncol, nlay, ngpt,
						ngas, nflav, ntemp, neta,
						nminorupper,
						nminorkupper,
				...
				const auto dt2=std::chrono::duration_cast<std::chrono::milliseconds>(t3-t2).count();

				cudaMemcpy(tau,my_tau,sizeof(Float)*dim,cudaMemcpyHostToDevice);
				free(my_tau);

				std::cout<<"Elapsed time for sequential kernel: "<<dt2<<" [ms]"<<std::endl;
