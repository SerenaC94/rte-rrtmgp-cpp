...
/*
#if use_shared_tau
template<typename Float, int block_size_x, int block_size_y, int block_size_z, int max_gpt=16> __global__
...
#if use_shared_tau == 0
template<int block_num_x, int block_num_y, int block_num_z, 
		...
		Float* __restrict__ tau_minor)
{
	for(int block_x=0;block_x<block_num_x;++block_x){
		for(int block_y=0;block_y<block_num_y;++block_y){
			for(int block_z=0;block_z<block_num_z;++block_z){
...
template<int block_num_x, int block_num_y, int block_num_z, 
	int block_size_x, int block_size_y, int block_size_z>
void gas_optical_depths_minor_reference_optimized_kernel(
		const int ncol, const int nlay, const int ngpt,
		const int ngas, const int nflav, const int ntemp, const int neta,
		const int nminor,
		const int nminork,
		...
		Float* __restrict__ tau_minor)
{
	...
	const auto tot_iterations=block_num_x*block_num_y*block_num_z;
	//Collapse the for in a block
	#pragma omp parallel for
	for(int iter=0;iter<tot_iterations;++iter){
		//for(int block_x=0;block_x<block_num_x;++block_x){
		//	for(int block_y=0;block_y<block_num_y;++block_y){
		//		for(int block_z=0;block_z<block_num_z;++block_z){
		Float scalings[block_size_z][block_size_y];
		for (int imnr=0; imnr<nminor; ++imnr){
			for(int thread_z=0;thread_z<block_size_z;++thread_z){
				for(int thread_y=0;thread_y<block_size_y;++thread_y){
					//This needs to be run only when thread_x==0
					//const int ilay = block_y * block_size_y + thread_y;
					//const int icol = block_z * block_size_z + thread_z;
					//Do first block_z then block_y
					//block_x=iter/(block_num_z*block_num_y)
					//block_y=(iter/(block_num_z)%block_num_y
					//block_z=iter%block_num_z
					const int ilay = (iter/block_num_z)%block_num_y*block_size_y+thread_y;
					const int icol = iter%block_num_z*block_size_z+thread_z;
					if ( (icol < ncol) && (ilay < nlay) )
					{
						const int idx_collay = icol + ilay*ncol;
			...
			for(int thread_z=0;thread_z<block_size_z;++thread_z){
				for(int thread_y=0;thread_y<block_size_y;++thread_y){
					...
					const int ilay = (iter/block_num_z)%block_num_y*block_size_y+thread_y;
					const int icol = iter%block_num_z*block_size_z+thread_z;
					//Move the operations indipendent from thread_x outside of the for
					//This gives roughly 1.75x speedup
					if ( (icol < ncol) && (ilay < nlay) )
					{
						const int idx_collay = icol + ilay*ncol;
